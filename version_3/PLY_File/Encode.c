...
void encodeWithNoise(Node **head, const char *str, int zLevel, int noiseAmount) {
    for (int i = 0; str[i] != '\0'; i++) {
        // Example pattern: a diagonal from (0,0,0) to (cubeSize-1, cubeSize-1, cubeSize-1)
        int x = (cubeSize - 1) * i / strlen(str);
        int y = (cubeSize - 1) * i / strlen(str);
        int z = (cubeSize - 1) * i / strlen(str);
